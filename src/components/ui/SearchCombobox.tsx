    ...
    query.trim() === ''
      ...
      : partidas.filter((partida) => {
          ...
          return searchTokens.every(
            (token) => descLower.includes(token) || codLower.includes(token)
          );
        });
    ...
    <div ref={wrapperRef} className="relative w-full group">
      <div className="relative">
        ...
        <input
          type="text"
          disabled={disabled}
          className="w-full pl-10 pr-10 h-[34px] rounded-xl border border-slate-200 bg-white shadow-sm focus:border-blue-400 focus:ring-4 focus:ring-blue-50 transition-all text-xs outline-none font-bold text-slate-700 disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed"
        ...
        />
        <ChevronDown
          className={`absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none transition-transform duration-300 ${
        ...
        <div className="absolute z-[110] w-full mt-2 bg-white/95 backdrop-blur-md border border-slate-200 rounded-2xl shadow-2xl max-h-64 overflow-auto animate-in fade-in zoom-in-95 duration-200 origin-top">
          {filteredPartidas.length === 0 ? (
            <div className="p-4 flex flex-col items-center gap-3">
              <div className="text-xs text-slate-400 text-center font-medium italic">
                Ninguna partida encontrada.
              </div>
              {onAddPartida && (
                <button
