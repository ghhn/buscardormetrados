...
// Helper para formatear fecha ISO (YYYY-MM-DD) a formato dd/mm/aa
const formatFechaExcel = (fechaISO) => {
    if (!fechaISO) return "";
    try {
        // Parsear fecha ISO (YYYY-MM-DD)
        const [year, month, day] = fechaISO.split('-');
        // Convertir a 2 dígitos para año
...
app.post('/api/export/metrados', async (req, res) => {
    try {
        ...
        // Ordenar metrados por fecha descendente (más reciente primero)
        const metradosOrdenados = metrados.sort((a, b) => {
            // Si ambos son templates o ambos no son, ordenar por fecha si existe
            ...
            // Templates primero, luego registros reales
            return a.is_template ? -1 : 1;
        });

        metradosOrdenados.forEach(m => {
            let rowData = null;
            ...
            // CASO B: Registro Real
                ...
                rowData[2] = formatFechaExcel(m.fecha); // C (Formato dd/mm/aa)
